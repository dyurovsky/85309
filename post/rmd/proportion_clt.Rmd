...
```{r sample_plots}
sample_distribution <- tibble(sample = 1:1000,
...
We expect from CLT that $\hat{p} \sim Normal\left(p, \sqrt{\frac{(p \left(1-p\right)}{n}}\right)$
...
pop_prop_estimate <- sample_distribution %>% 
  pull(prop) %>% 
  mean()
pop_prop_estimate

sample_prop_sd <- sample_distribution %>% 
  pull(prop) %>%
  sd()
...
lower_quantile <- sample_distribution %>%
  pull(prop) %>%
  quantile(.025)

upper_quantile <- sample_distribution %>%
  pull(prop) %>%
  quantile(.975)
...
ggplot(sample_distribution, aes(x = prop)) + 
  ...
  geom_vline(aes(xintercept = lower_quantile), 
             color = "#bb0000", size = 2) +
  geom_vline(aes(xintercept = upper_quantile), 
             color = "#bb0000", size = 2)
```
...
sample_prop_quantiles <- sample_distribution %>%
  pull(prop) %>%
...
```{r estimate_null_cis}
null_distribution <- tibble(sample = 1:1000,
                         prop = replicate(1000, sample_prop(.75)))

ggplot(null_distribution, aes(x = prop)) + 
  geom_histogram(binwidth = .0075)


null_quantiles <- null_distribution %>%
  pull(prop) %>%
  quantile(probs = c(.025, .975))
